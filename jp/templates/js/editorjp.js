...
Vue.component('editorjp', {
  ...
  mounted() {
    let simplemde = new SimpleMDE({element: (this.$refs['r' + this.$props.jp_props.id])});
  ...
  updated() {
    if (this.$props.jp_props.input_type) {    //this.$props.jp_props.input_type
      if (this.$props.cached_value !== this.$props.jp_props.value) {
        let cursor_position = this.$props.simplemde.codemirror.getCursor();
